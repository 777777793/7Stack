...
分块策略至少有五种。固定大小分块：按 token 数均匀切分（如 256 token 一块），通常会设一定的 overlap（如 20 token）防止语义被切成两半。句子分块：按句号、换行等自然分隔符切分，保证每个 chunk 是一个完整句子。段落分块：以段落为边界。语义分块：用一个小模型计算相邻句子之间的 embedding 相似度，通过相似度的"断崖下降"来找出语义的自然分界点。层级分块：把文档按标题层级拆成树形结构，检索时可以定位到更精确的章节。
